...
# 3. 输入 linda (全英文，使用 type_text 结合一次 Enter 确认输入法)
step("输入 'linda'")
gi.type_text("linda", use_clipboard=False)
time.sleep(1.0)
gs.screenshot(save_path=os.path.join(SCREENSHOT_DIR, "wx_3_typed.png"))

step("按一次 Enter 确认输入法的英文字符")
gi.key_press("enter")
time.sleep(1.0)
gs.screenshot(save_path=os.path.join(SCREENSHOT_DIR, "wx_4_entered_ime.png"))

# 4. 选择并进入联系人
step("按 Enter 直接进入已选中的第一个联系人聊天")
# 对于普通联系人，微信会自动选中“联系人”分类的第一条，所以不需要Down，直接Enter即可
time.sleep(0.5)
